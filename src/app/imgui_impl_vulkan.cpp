...
struct ImGui_ImplVulkan_ViewportData
{
    ...
    ImGui_ImplVulkanH_WindowRenderBuffers RenderBuffers;
    bool                                WindowOwned;
...
static void ImGui_ImplVulkanH_DestroyWindowRenderBuffers(VkDevice device, ImGui_ImplVulkanH_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator);
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    ImGui_ImplVulkan_ViewportData* viewport_data = (ImGui_ImplVulkan_ViewportData*)draw_data->OwnerViewport->RendererUserData;
    IM_ASSERT(viewport_data != nullptr);
    ImGui_ImplVulkanH_WindowRenderBuffers* wrb = &viewport_data->RenderBuffers;
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    err = vkCreateCommandPool(v->Device, &pool_info, v->Allocator, &command_pool);
    check_vk_result(err);
    ...
    err = vkAllocateCommandBuffers(v->Device, &alloc_info, &command_buffer);
    check_vk_result(err);
    ...
    // Copy to Image:
    {
        ...
        vkCmdCopyBufferToImage(command_buffer, upload_buffer, bd->FontImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    ...
    vkDestroyCommandPool(v->Device, command_pool, v->Allocator);
...
static void ImGui_ImplVulkan_CreateFontSampler(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ...
    // Bilinear sampling is required by default. Set 'io.Fonts->Flags |= ImFontAtlasFlags_NoBakedLines' or 'io.Fonts->Flags |= ImFontAtlasFlags_NoPowerOfTwoHeight' to allow point/nearest sampling.
...
void ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (bd->FontDescriptorSet)  { bd->FontDescriptorSet = VK_NULL_HANDLE; } // Freed by DescriptorPool
...
bool ImGui_ImplVulkan_LoadFunctions(PFN_vkVoidFunction(*loader_func)(const char* function_name, void* user_data), void* user_data)
{
    ...
    // You can use this default Vulkan loader if you link with Vulkan SDK libraries and have them installed
    // Alternatively, you can grab function pointers from glfwGetInstanceProcAddress() if using GLFW
    (void)loader_func;
    (void)user_data;
    return true;
}
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info)
{
    ...
    IM_ASSERT(sizeof(ImTextureID) >= sizeof(ImU64) && "Can't pack descriptor set handle into TexID, 32-bit not supported yet.");

    bd->VulkanInitInfo = *info;

    ImGui_ImplVulkan_CreateDeviceObjects();

    // Create main viewport render data
    ImGuiViewport* main_viewport = ImGui::GetMainViewport();
    ImGui_ImplVulkan_ViewportData* vd = IM_NEW(ImGui_ImplVulkan_ViewportData)();
    vd->WindowOwned = false;
    main_viewport->RendererUserData = vd;

    // Setup multi-viewport capabilities
...
void ImGui_ImplVulkan_Shutdown()
{
    ...
    // Clean up multi-viewport support
    if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // Select Present Mode
    // Request a certain mode if possible, fallback to VK_PRESENT_MODE_FIFO_KHR which is mandatory
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    {
        err = vkWaitForFences(v->Device, 1, &fd->Fence, VK_TRUE, UINT64_MAX);    // wait indefinitely instead of periodically checking
        check_vk_result(err);
...
static void ImGui_ImplVulkan_SwapBuffers(ImGuiViewport* viewport, void*)
{
    ...
    if (err == VK_ERROR_OUT_OF_DATE_KHR || err == VK_SUBOPTIMAL_KHR)
        ImGui_ImplVulkan_SetWindowSize(viewport, viewport->Size);
    else
        check_vk_result(err);
...
static void ImGui_ImplVulkan_InitPlatformInterface()
{
    ...
    if (ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
        IM_ASSERT(ImGui::GetIO().BackendFlags & ImGuiBackendFlags_PlatformHasViewports);
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    // If min_image_count was not specified, request different count of images dependent on selected present mode
    ...
    // Create The Image Views
    {
        for (uint32_t i = 0; i < wd->ImageCount; i++)
        {
            VkImageViewCreateInfo info = {};
            info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
            info.image = wd->Frames[i].Backbuffer;
            info.viewType = VK_IMAGE_VIEW_TYPE_2D;
            ...
            err = vkCreateImageView(device, &info, allocator, &wd->Frames[i].BackbufferView);
            check_vk_result(err);
        }
    }
    ...
    if (wd->UseDynamicRendering == false)
    {
        for (uint32_t i = 0; i < wd->ImageCount; i++)
        {
            VkImageView attachment[1] = { wd->Frames[i].BackbufferView };
            ...
            info.layers = 1;
            err = vkCreateFramebuffer(device, &info, allocator, &wd->Frames[i].Framebuffer);
            check_vk_result(err);
        }
    }
}

// Create or recreate the window objects within wd (ImGui_ImplVulkanH_Window)
// - the pipeline is expected to be destroyed by user before calling this (e.g. the ImGui_ImplVulkan_CreatePipeline() function)
// - the command pool, command buffer, fence, semaphore are expected to be destroyed by user before calling this
// - this function only destroys and recreates them
